variable "name_config_validation" {
  ...
  validation {
    condition = alltrue([
      for nc in var.name_config_validation.name_configs : nc.parent_name != null || (
        contains(keys(nc.values), "environment")
        && can(contains(var.name_config_validation.allowed_environments, nc.values.environment))
      )
    ])
  ...
  validation {
    ...
    error_message = "Every name_configs value must contain a 'workload_name' key that is a lowercase alphanumeric string with hyphens"
  }
